...
def example_1(n=10000 , d=2, env =1): 
    ...
    # Confounding version
    # confounder = torch.randn(n, d) 
    # x1 = confounder + torch.randn(n, d) * env
    # y = x1 + torch.randn(n, d) * env
    # return torch.cat((x1, confounder), 1), y.sum(1, keepdim=True) 

n_var = 2
dim = 10
phi = torch.nn.Parameter(torch.ones(n_var* dim, 1)) 
dummy_w = torch.nn.Parameter(torch.Tensor([1.0])) 
opt = torch.optim.SGD([phi], lr=1e-3) 
mse = torch.nn.MSELoss(reduction="none") 
environments = [example_1(d=dim, env=0.1), example_1 (d=dim, env =1.0)] 
